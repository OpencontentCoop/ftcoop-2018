$.ftcSocieta = {
  render: function(settings){
    ...
    var renderMap = function(container, geoJson){
      ...
      if (listContainer.length > 0 && geoJson.features && geoJson.features.length > 0) {
        var list = $('<ul class="simple"></ul>');
        $.each(geoJson.features, function(){
            var row = $('<li data-marker="'+this.id+'"></li>');
            var content = '<p><strong>'+this.properties.name+'</strong></p>';
            ...
            row.css('cursor', 'pointer').on('click', function(e){                
                ...
                markers.eachLayer(function (layer) {
                  if (layer.feature && layer.feature.id == itemClicked.data('marker')){                    
        ...
        listContainer.css('max-height', mapContainer.height() - 40);
          listContainer.css('overflow-y', 'auto');
          listContainer.css('margin', '20px 0');
        /*listContainer.hide();
  ...
  renderMapOnly: function(settings){
      var renderMap = function(container, geoJson){
          var tiles = L.tileLayer('//{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {maxZoom: 18,attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'});
          var map = L.map(container).addLayer(tiles)
          map.scrollWheelZoom.disable();
