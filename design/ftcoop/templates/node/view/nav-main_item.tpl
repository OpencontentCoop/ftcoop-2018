...
{if and(gt($sub_menu_items|count(),0), $show_dropdown)}
  ...
  {foreach $sub_menu_items as $subitem}
    ...
    {if or(eq( $subitem.class_identifier, 'link'),eq( $subitem.class_identifier, 'servizio_intracoop'))}
      <li id="node_id_{$subitem.node_id}" {if $subitem_class} class="{$subitem_class|implode(" ")}"{/if}>
        <a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $subitem.node_id)|ezurl}{else}{if $subitem.data_map.location.has_content}href={$subitem.data_map.location.content|ezurl}{else}href={$subitem.url_alias|ezurl()}{/if}{if and( is_set( $subitem.data_map.open_in_new_window ), $subitem.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $pagedata.is_edit} onclick="return false;"{/if} title="{$subitem.data_map.location.data_text|wash}" rel={$subitem.url_alias|ezurl}>{if $subitem.data_map.location.data_text}{$subitem.data_map.location.data_text|wash()}{else}{$subitem.name|wash()}{/if}</a>
      </li>
    {else}
      <li id="node_id_{$subitem.node_id}" {if $subitem_class} class="{$subitem_class|implode(" ")}"{/if}>
        <a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $subitem.node_id)|ezurl}{else}{$subitem.url_alias|ezurl}{/if}{if $pagedata.is_edit} onclick="return false;"{/if}>{$subitem.name|wash()}</a>
      </li>
    {/if}
    {undef $subitem_class}
  {/foreach}
