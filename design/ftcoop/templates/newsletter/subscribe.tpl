...
<div class="container">
  ...
  <div class="content-view-full newsletter newsletter-subscribe row">

      <div class="content-main wide">



  {* gestione dell'email: se viene passato un parametro via post e le altre condizioni non sono verificate, precompila il campo e-mail *}
  {def $this_email = ''}

  {if and( is_set( $user ), $subscription_data_array['email']|eq('') )}
    {* se l'utente ha effettuato il login, prende l'email dell'utente *}
    {set $this_email = $user.email|wash()}
  {elseif $subscription_data_array['email']}
    {* se c'è un valore email nell'array di iscrizione, prende quella *}
    {set $this_email = $subscription_data_array['email']|wash )}
  {elseif ezhttp_hasvariable( 'Subscription_Email', 'post' )}
    {set $this_email = ezhttp( 'Subscription_Email', 'post' )}
  {/if}
			      ...
			      <div class="col-sm-7">
			        <div class="abstract">
			          ...
			          <p>{'Please fill in the boxes "first name" and "last name" and enter your e-mail address in the corresponding field. Then, select the newsletter you are interested in and the format you prefer.'|i18n( 'cjw_newsletter/subscribe' )}</p>
						...
						<div class="col-sm-5">
							<h3 style="margin-top: 0;">Leggi le ultime newsletter</h3>
							<div class="well">
								{include uri='design:parts/archivio_newsletter.tpl'}
							</div>
              ...
              {def $newsletter_list_node_list = fetch( 'content', 'tree',
                   ...
                   $newsletter_list_node_list_count = $newsletter_list_node_list|count}
                    ...
                    {if $newsletter_list_node_list_count|ne(0)}
                        ...
                        {if $newsletter_list_node_list_count|gt(1)}
                                ...
                                <tr>
                                    ...
                                    <td valign="top" class="newsletter-list">
                                        ...
                                        {if $newsletter_list_node_list_count|eq(1)}
                                            <h4>{$system_node.data_map.title.content|wash} - {$list_node.data_map.title.content|wash}</h4>
                                            <input class="hidden" type="checkbox" name="Subscription_ListArray[]" value="{$list_id}" checked="checked" title="{$list_node.data_map.title.content|wash}" /> {*$list_node.data_map.title.content|wash*}
                                        {else}
            ...
            *}
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                  ...
                  <input class="form-control" id="Subscription_Email" type="text" name="Subscription_Email" value="{$this_email}" title="{'Email of the subscriber.'|i18n( 'cjw_newsletter/subscribe' )}" />
                </div>
            </div>
            ...
            <div class="form-group">
              <div class="col-sm-offset-2 col-sm-10">
                ...
                <input class="btn btn-primary" type="submit" name="SubscribeButton" value="{'Subscribe'|i18n( 'cjw_newsletter/subscribe' )}" title="{'Add to subscription.'|i18n( 'cjw_newsletter/subscribe' )}" />
                {*<a href={$node_url|ezurl}><input class="btn btn-danger" type="submit" name="CancelButton" value="{'Cancel'|i18n( 'cjw_newsletter/subscribe' )}" /></a>*}
              </div>
            </div>
