...
<div class="nav-section">
  <button type="button" class="btn btn-default visible-xs visible-sm" data-toggle="collapse" data-target="#nav-section-inner">
    Naviga la sezione <i class="fa fa-angle-down"></i>
  </button>
  <div class="nav-section-inner navbar-collapse collapse" id="nav-section-inner">
	{if $menu_items_count}
	<ul class="nav">
	  {foreach $menu_items as $key => $item}
		...
		{else}
		  <li {if $item.node_id|eq($node.node_id)}class="active"{/if}>
                ...
                {if $sub_menu_items_count}
                    <ul class="nav-sub">
                        {foreach $sub_menu_items as $subkey => $subitem}
                          ...
                          {else}
                              <li {if $subitem.node_id|eq($node.node_id)}class="active"{/if}>
                                <a href="{$subitem.url_alias|ezurl( 'no' )}">
                                {$subitem.name|wash()}
                              </a></li>
                          {/if}
                        {/foreach}
