...
{def $comunicati_search = fetch( 'ezfind', 'search', hash(
                                                    'subtree_array', array( ezini( 'NodiRilevanti', 'ComunicatiStampa', 'ftcoop.ini' ) ),
                                                    'class_id', array( 'comunicato' ),
                                                    'limit', 6,
                                                    'sort_by', hash( solr_field('from_time', 'date'), 'desc', solr_field('publish_date', 'date'), 'desc', 'published', 'desc' )
                                                    ) )
     $comunicati = cond( $comunicati_search.SearchCount|gt(0), $comunicati_search.SearchResult, array() )}

{def $webtv_search = fetch( 'ezfind', 'search', hash(
                                                    'subtree_array', array( ezini( 'NodiRilevanti', 'WebTV', 'ftcoop.ini' ) ),
                                                    'class_id', array( 'web_tv' ),
                                                    'limit', 3,
                                                    'sort_by', hash( solr_field('publish_date', 'date'), 'desc', 'published', 'desc' )
                                                    ) )
     $webtv = cond( $webtv_search.SearchCount|gt(0), $webtv_search.SearchResult, array() )}

{set $comunicati = $comunicati|merge( $webtv )}
{set $valid_nodes = $comunicati|merge( $valid_nodes )}
