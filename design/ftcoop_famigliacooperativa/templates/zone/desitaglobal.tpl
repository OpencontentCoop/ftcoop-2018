...
{def $current_node = $attribute.object.main_node
     $openpa = object_handler($current_node)}
    ...
    <div class="sub-header bg-primary">
      <div class="container-fluid">
        <div class="col-md-11 col-md-offset-1 col-xs-offset-0 col-sm-offset-0 showcase">
          {block_view_gui block=$block_wrapper.block items_per_row=$block_wrapper.items_per_row link_top_title=false() is_subheader=true()}
        </div>
      </div>
    </div>  

    {if $openpa.content_tools.editor_tools}
      {include uri=$openpa.content_tools.template node=$current_node}
    {/if}

  {else}

    <section class="page-section{if $block_wrapper.is_wide}-wide{/if}{if $block_wrapper.has_style} u-background-grey-20{/if}">
      
      {if $block_wrapper.is_wide|not()}
      <div class="container">
        <div class="row">
          <div class="max-width">
      {/if}
