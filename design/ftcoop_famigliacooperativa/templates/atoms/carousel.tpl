{set_defaults( hash(
    ...
    'navigation_text', concat( "['", '<i class="fa fa-angle-left" role="presentation"></i>', "','", '<i class="fa fa-angle-right" role="presentation"></i>', "']"),
    'css_id', $root_node.node_id
))}
{if and( $root_node, $items )}
    {if $root_node}

    <div id="carousel_{$css_id}" class="owl-carousel owl-theme showcase">
            ...
            $("#carousel_{$css_id}").initOwlCarousel(
                {ldelim}
                    ...
                    {if $top_pagination_position}, onInitialized: function(e){ldelim}
                      ...
                      owl.find('.owl-dots').addClass('hidden-md hidden-lg');
                      {/if}
                      {rdelim}
                    {/if}
                {rdelim},
